...
// Función para abrir el video (puedes usar un simple window.open o un Modal)
// Función maestra para abrir videos
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (val.length < 2) {listaTop.style.display = 'none'; return; }
    ...
    fetch(`/ProductoPublic/Buscar?termino=${encodeURIComponent(val)}`)
        ...
        listaTop.innerHTML = data.map(p => `
                        <a href="/ProductoPublic/ProductoDetalle?area=${p.area}&nombre=${encodeURIComponent(p.descripcion)}" 
                           class="d-flex align-items-center p-2 text-decoration-none border-bottom hover-bg text-dark">
    ...
    document.addEventListener('click', (e) => { if(!inputTop.contains(e.target)) listaTop.style.display = 'none'; });
